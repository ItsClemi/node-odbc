	...
	describe( `query type checker insert / fetch (exec toSingle)`, function ()
	{
		...
		it( "insert null -> fetch null", ( done ) =>
		{
			...
			async.series( [
				( cb ) =>
				{
					con.executeQuery( odbc.eFetchMode.eSingle, ( res, err ) => {
							cb( err );
						},
				...
				( cb ) =>
				{
					con.executeQuery<{a: number | null}>(
						odbc.eFetchMode.eSingle, 
						( res, err ) =>
						{
							assert( res.a == null );
			...
			async.series( [
				...
				( cb ) =>
				{
					con.executeQuery(
						...
						`INSERT INTO ${tblName}(a)VALUES(?)`, odbc.makeTimestamp( date )
					);
				},

				( cb ) =>
				{
					con.executeQuery<{ a: Date }>(
						odbc.eFetchMode.eSingle,
						( res, err ) =>
						{
							if( res.a == undefined )
							...
							assert( res.a.getFullYear() == date.getFullYear() );
							assert( res.a.getMonth() == date.getMonth() );
							assert( res.a.getDate() == date.getDate() );
